...
use crate::{BufferMemoryBarrier, GlobalMemoryBarrier, ImageMemoryBarrier, SyncLayout};
...
pub fn get_image_memory_barrier<'a>(
    ...
    let old_layout = if let Some(barrier_old_layout) = barrier.old_layout {
        match barrier_old_layout {
            SyncLayout::Optimal => previous_info.image_layout,
            SyncLayout::Manual(layout) => layout.to_erupt(),
        }
    } else {
    ...
    let new_layout = match barrier.new_layout {
        SyncLayout::Optimal => next_info.image_layout,
        SyncLayout::Manual(layout) => layout.to_erupt(),
    };
